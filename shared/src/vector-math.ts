...
export function clampVector(v: PoliticalVector): PoliticalVector {
  return {
...
export function euclideanDistance(a: PoliticalVector, b: PoliticalVector): number {
...
export function evaluateThresholds(distance: number, config: ThresholdConfig): InterventionLevel {
  if (distance < config.echoChamber) return "critical";
  if (distance < config.mildBias) return "medium";
  if (distance >= config.diverseExposure) return "none";
  return "low";
}
