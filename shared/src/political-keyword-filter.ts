...
const SENSITIVITY_THRESHOLDS: Record<PoliticalSensitivity, number> = {
  low: 10,
  medium: 3,
  high: 1,
};

const OBFUSCATION_MAP: Readonly<Record<string, string>> = {
  "0": "o",
  "1": "i",
  "3": "e",
  "4": "a",
...
function applyObfuscationMap(text: string): string {
  let normalized = text;
...
const normalizedKeywordEntries = POLITICAL_KEYWORDS.map((keyword) => {
  ...
  return [normalized, keyword] as const;
}).filter((entry) => entry[0].length > 0);

const canonicalKeywordMap = new Map<string, string>(normalizedKeywordEntries);
const normalizedSingleWordKeywords = new Set<string>();
const normalizedPhraseKeywords: string[] = [];
const normalizedDevoweledKeywords = new Map<string, string>();
...
function computeConfidence(matchCount: number, tokenCount: number): number {
...
export function isPolitical(
  text: string,
  sensitivity: PoliticalSensitivity = "medium"
): PoliticalDetectionResult {
