...
type Phase1StructuredResponse = {
  tweet_vector: PoliticalVector;
  fallacies: string[];
  topic: string;
  confidence: number;
};
...
type Phase1AnalyzerLogger = {
  info: (message: string) => void;
  warn: (message: string) => void;
};

type Phase1AnalyzerOptions = {
  apiKey?: string;
...
const buildPhase1Prompt = (tweetText: string): string => {
  return [
    "You are a lightweight political tweet analyzer.",
...
const toStrictStructured = (value: unknown): Phase1StructuredResponse | null => {
  if (!isObject(value)) {
    return null;
  }
...
const requestGemini = async (
  ...
  logger: Phase1AnalyzerLogger,
  now: () => number
): Promise<Phase1StructuredResponse | null> => {
  ...
  try {
    ...
    logger.info(
      `[phase1-analyzer] request_ms=${durationMs} tokens_total=${tokenUsage} prompt_tokens=${responseBody?.usageMetadata?.promptTokenCount ?? 0} completion_tokens=${responseBody?.usageMetadata?.candidatesTokenCount ?? 0}`
    );
...
export const createPhase1Analyzer = (options: Phase1AnalyzerOptions = {}) => {
  ...
  return async (_tweetId: string, tweetText: string): Promise<Phase1Analysis | null> => {
    ...
    if (apiKey.length === 0) {
      logger.warn("[phase1-analyzer] GEMINI_API_KEY missing, skipping analyzer");
      return null;
    }

    const truncated = truncateDeterministically(trimmedText, maxInputChars);
    const prompt = buildPhase1Prompt(truncated);
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
      try {
        ...
        if (!retryableError || !shouldRetry(retryableError) || !hasNextAttempt) {
          logger.warn("[phase1-analyzer] failed after retries, returning null fallback");
          return null;
        }

        const backoffMs = calculateBackoff(attempt, retryableError);
        logger.warn(`[phase1-analyzer] retry attempt=${attempt + 1} backoff_ms=${backoffMs}`);
...
export const phase1AnalyzeTweet = createPhase1Analyzer();
